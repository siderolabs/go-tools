...
#
# Generated on 2026-06-08T08:07:12Z by kres a07fce6.
...
ARG SHA
ARG TAG
RUN --mount=type=cache,target=/root/.cache/go-build,id=go-tools/root/.cache/go-build --mount=type=cache,target=/go/pkg,id=go-tools/go/pkg GOARCH=amd64 GOOS=darwin go build ${GO_BUILDFLAGS} -gcflags "${GO_GCFLAGS}" -ldflags "${GO_LDFLAGS} -X ${VERSION_PKG}.Name=extensions-duplicate-finder -X ${VERSION_PKG}.SHA=${SHA} -X ${VERSION_PKG}.Tag=${TAG}" -o /extensions-duplicate-finder-darwin-amd64
...
ARG SHA
ARG TAG
RUN --mount=type=cache,target=/root/.cache/go-build,id=go-tools/root/.cache/go-build --mount=type=cache,target=/go/pkg,id=go-tools/go/pkg GOARCH=arm64 GOOS=darwin go build ${GO_BUILDFLAGS} -gcflags "${GO_GCFLAGS}" -ldflags "${GO_LDFLAGS} -X ${VERSION_PKG}.Name=extensions-duplicate-finder -X ${VERSION_PKG}.SHA=${SHA} -X ${VERSION_PKG}.Tag=${TAG}" -o /extensions-duplicate-finder-darwin-arm64
...
ARG SHA
ARG TAG
RUN --mount=type=cache,target=/root/.cache/go-build,id=go-tools/root/.cache/go-build --mount=type=cache,target=/go/pkg,id=go-tools/go/pkg GOARCH=amd64 GOOS=linux go build ${GO_BUILDFLAGS} -gcflags "${GO_GCFLAGS}" -ldflags "${GO_LDFLAGS} -X ${VERSION_PKG}.Name=extensions-duplicate-finder -X ${VERSION_PKG}.SHA=${SHA} -X ${VERSION_PKG}.Tag=${TAG}" -o /extensions-duplicate-finder-linux-amd64
...
ARG SHA
ARG TAG
RUN --mount=type=cache,target=/root/.cache/go-build,id=go-tools/root/.cache/go-build --mount=type=cache,target=/go/pkg,id=go-tools/go/pkg GOARCH=arm64 GOOS=linux go build ${GO_BUILDFLAGS} -gcflags "${GO_GCFLAGS}" -ldflags "${GO_LDFLAGS} -X ${VERSION_PKG}.Name=extensions-duplicate-finder -X ${VERSION_PKG}.SHA=${SHA} -X ${VERSION_PKG}.Tag=${TAG}" -o /extensions-duplicate-finder-linux-arm64
...
ARG SHA
ARG TAG
RUN --mount=type=cache,target=/root/.cache/go-build,id=go-tools/root/.cache/go-build --mount=type=cache,target=/go/pkg,id=go-tools/go/pkg GOARCH=amd64 GOOS=darwin go build ${GO_BUILDFLAGS} -gcflags "${GO_GCFLAGS}" -ldflags "${GO_LDFLAGS} -X ${VERSION_PKG}.Name=image-signer -X ${VERSION_PKG}.SHA=${SHA} -X ${VERSION_PKG}.Tag=${TAG}" -o /image-signer-darwin-amd64
...
ARG SHA
ARG TAG
RUN --mount=type=cache,target=/root/.cache/go-build,id=go-tools/root/.cache/go-build --mount=type=cache,target=/go/pkg,id=go-tools/go/pkg GOARCH=arm64 GOOS=darwin go build ${GO_BUILDFLAGS} -gcflags "${GO_GCFLAGS}" -ldflags "${GO_LDFLAGS} -X ${VERSION_PKG}.Name=image-signer -X ${VERSION_PKG}.SHA=${SHA} -X ${VERSION_PKG}.Tag=${TAG}" -o /image-signer-darwin-arm64
...
ARG SHA
ARG TAG
RUN --mount=type=cache,target=/root/.cache/go-build,id=go-tools/root/.cache/go-build --mount=type=cache,target=/go/pkg,id=go-tools/go/pkg GOARCH=amd64 GOOS=linux go build ${GO_BUILDFLAGS} -gcflags "${GO_GCFLAGS}" -ldflags "${GO_LDFLAGS} -X ${VERSION_PKG}.Name=image-signer -X ${VERSION_PKG}.SHA=${SHA} -X ${VERSION_PKG}.Tag=${TAG}" -o /image-signer-linux-amd64
...
ARG SHA
ARG TAG
RUN --mount=type=cache,target=/root/.cache/go-build,id=go-tools/root/.cache/go-build --mount=type=cache,target=/go/pkg,id=go-tools/go/pkg GOARCH=arm64 GOOS=linux go build ${GO_BUILDFLAGS} -gcflags "${GO_GCFLAGS}" -ldflags "${GO_LDFLAGS} -X ${VERSION_PKG}.Name=image-signer -X ${VERSION_PKG}.SHA=${SHA} -X ${VERSION_PKG}.Tag=${TAG}" -o /image-signer-linux-arm64
